...
import type { Ticket, Event, TicketType, User, RedeemFailureReason, RedeemTicketResponse } from '../../../types';
...
const ScanModal: React.FC<{
  ...
  const handleVerification = useCallback(async (qrCodeValue: string) => {
    ...
    try {
        ...
        // Fix: Inverted the conditional to help TypeScript's type narrowing on the discriminated union `RedeemTicketResponse`.
        if (!result.success) {
            setVerificationResult({ status: 'error', message: result.message, reason: result.reason });
        ...
        const detectionInterval = setInterval(async () => {
            if (videoRef.current && videoRef.current.readyState === 4 && streamRef.current) {
                try {
                    ...
                    // Ignore detection errors, which can happen between frames
                }
            }
        }, 500);
        ...
        return (
          <div className="text-center">
            ...
            <div className="text-left bg-gray-100 dark:bg-gray-700 p-4 rounded-lg mt-4 space-y-2 text-sm">
                <p><strong>Event:</strong> {verificationResult.data?.event.title}</p>
                <p><strong>Ticket:</strong> {verificationResult.data?.ticketType.name}</p>
                <p><strong>Attendee:</strong> {verificationResult.data?.user.name}</p>
            </div>
