            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 max-w-md w-full relative">
                ...
                {!result ? (
                    <form onSubmit={handleVerify} className="mt-6 space-y-4">
                        <p className="text-gray-600 dark:text-gray-400 text-center text-sm">Enter the ticket code to verify its authenticity.</p>
                        <div>
...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ topSellingEvents }) => {
    const [isVerificationModalOpen, setIsVerificationModalOpen] = useState(false);
    return (
        <>
            <aside className="lg:col-span-1 space-y-8">
                <TopEventsCard events={topSellingEvents} title="Top Selling Events" />
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                        <div>
                            <h3 className="font-bold text-lg text-gray-900 dark:text-white">Scan & Verify Tickets</h3>
                            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Verify attendee tickets for any event.</p>
                        </div>
                        <button
